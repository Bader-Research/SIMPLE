...
#if TEST_WIDE
#define ARR_SIZE_SORT   ((long)1<<33)
#else
...
void
test_radixsort_smp(long N1, THREADED) {
  int *inArr, *outArr;
  ...
  inArr  = (int *)node_malloc_l(N1 * sizeof(int), TH);
  outArr = (int *)node_malloc_l(N1 * sizeof(int), TH);
  ...
  on_one {
    fprintf(stdout,"T: %3d n: %13ld  SSort: %9.6lf  (MB:%5d)\n",
	    THREADS,N1,tsec,
